...
import com.anupawar.appupdate.Utils.PackageUtils;
...
public class UpdateDialog extends AbstractFragment implements View.OnClickListener {
    ...
    private void isLatest() {
        if (mIsShowToast) {
            TastyToast.makeText(getActivity(),TextUtils.isEmpty(mToastMsg) ? getResources().getString(R.string.update_lib_default_toast) : mToastMsg,TastyToast.LENGTH_SHORT,TastyToast.ERROR);
    ...
    protected void initIfMustUpdate(View view, int id) {
        if (PackageUtils.getVersionCode(mActivity.getApplicationContext()) < mModel.getMinSupport()) {
            ...
            FunctionUtils.setLastCheckTime(getActivity().getApplicationContext(), 0);
        }
    }
